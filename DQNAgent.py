...
class DQNAgent:
    def __init__(self, env,
        ...
        # joint action 차원: product of all sub-action dims
        ...
        # Q-network & target
        self.qnet = ConvQNet(obs_shape, hidden_dim, self.n_actions).to(device)
        self.target = ConvQNet(obs_shape, hidden_dim, self.n_actions).to(device)
    ...
    def get_action(self, state, is_training=True):
        ...
        if is_training and self.step_count > self.learning_starts:
            self.epsilon = max(
                self.eps_end,
                self.eps_start - (self.step_count - self.learning_starts) * (self.eps_start - self.eps_end) / self.eps_decay
            )
        ...
        # greedy joint action 선택
        with torch.no_grad():
            x = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            qvals = self.qnet(x).squeeze(0)         # [n_actions]
            idx   = qvals.argmax().item()          # flat index
        # multi-dim action 복원
    ...
    def update(self):
        ...
        # 현재 Q(s,a)
        qvals      = self.qnet(states)                       # [B, n_actions]
        current_q  = qvals.gather(1, actions).squeeze(1)     # [B]

        # 타깃 Q
        with torch.no_grad():
            next_qvals = self.target(next_states)            # [B, n_actions]
            max_next   = next_qvals.max(dim=1)[0]            # [B]
